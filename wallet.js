...
async function listTransactions() {
    try {
        ...
        if (response.data.status !== '1') {
            throw new Error(`Could not fetch history. API Message: ${response.data.message} | ${response.data.result}`);
        }
        ...
        if (receivedTransactions.length === 0) {
            console.log("No received transactions found for this address.");
        } else {
        ...
        if (error.response && error.response.status === 502) {
            console.error("\nCould not communicate with the QuaiScan server (Error 502). This is usually a temporary issue on their end. Please try again later.");
...
async function sendFromDerivedWallet() {
    try {
        ...
        const senderAddress = process.env.DERIVED_ADDRESS;
...
async function sendToDerivedWallet() {
    try {
        ...
        const senderWallet = quais.QuaiHDWallet.fromMnemonic(senderMnemonic);
        await senderWallet.connect(provider);
        const senderAddress = process.env.MAIN_ADDRESS;
        ...
        const configuredBip39 = await getBip39Passphrase(); // Get the configured passphrase to compare
